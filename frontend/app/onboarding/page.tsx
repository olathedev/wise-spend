...
const COACH_PERSONAS = [
    { id: 'drill_sergeant', label: 'Drill Sergeant', desc: 'Strict, aggressive accountability.', icon: Target, color: 'bg-slate-900 text-white' },
    { id: 'cheerleader', label: 'Cheerleader', desc: 'Positive reinforcement & celebration.', icon: Sparkles, color: 'bg-pink-500 text-white' },
    { id: 'analyst', label: 'The Analyst', desc: 'Pure logic, data-driven insights.', icon: TrendingUp, color: 'bg-blue-600 text-white' },
];
                    ...
                    {step === 1 && (
                        ...
                        >
                            <div className="text-left">
                                <h2 className="text-3xl font-semibold text-slate-900">Let's start with a baseline</h2>
                                <p className="text-slate-500 mt-2">What is your approximate monthly net income?</p>
                            </div>

                            <div className="relative">
                                <span className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-900 text-2xl font-medium">$</span>
                                <input
                                    type="number"
                                    placeholder="0.00"
                    ...
                    {step === 2 && (
                        ...
                        >
                            <div className="text-left mb-6">
                                <h2 className="text-3xl font-semibold text-slate-900">What's your main focus?</h2>
                                <p className="text-slate-500 mt-2">Select up to 5 goals that matter most.</p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {GOALS.map((goal) => (
                                    <button
                                        ...
                                        className={`p-4 rounded-xl border transition-all text-center text-sm font-medium ${formData.financialGoals.includes(goal.id)
                    ...
                    {step === 3 && (
                        ...
                        >
                            <div className="text-left mb-8">
                                <h2 className="text-3xl font-semibold text-slate-900">Pick your AI Coach</h2>
                                <p className="text-slate-500 mt-2">How should I keep you on track?</p>
                            </div>
                            ...
                            <div className="space-y-3">
                                {COACH_PERSONAS.map((persona) => (
                                    <button
                                        key={persona.id}
                                        onClick={() => setFormData({ ...formData, coachPersonality: persona.id })}
                                        className={`w-full p-4 rounded-2xl border transition-all flex items-center gap-4 text-left ${formData.coachPersonality === persona.id
                                            ? 'border-teal-500 bg-teal-50 shadow-md transform scale-[1.02]'
                                            : 'border-gray-100 hover:border-teal-200 hover:bg-slate-50'
                                            }`}
                                    >
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-sm ${persona.color}`}>
                ...
                <div className="mt-12 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        {[1, 2, 3].map((s) => (
                            <div
                                key={s}
                                className={`h-2.5 w-2.5 rounded-full transition-colors duration-300 ${s <= step ? 'bg-teal-500' : 'bg-gray-200'
                                    }`}
                            />
                        ))}
                    </div>
