...
const GoalCard: React.FC<GoalCardProps> = ({ goal }) => {
  ...
  return (
    <div className="bg-card-light dark:bg-card-dark p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm relative group hover:shadow-md transition-shadow">
      ...
      <h4 className="text-lg font-bold text-black dark:text-dark">{goal.name}</h4>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{goal.description}</p>
      
      <div className="flex justify-between items-end">
        <div>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Current / Target</p>
